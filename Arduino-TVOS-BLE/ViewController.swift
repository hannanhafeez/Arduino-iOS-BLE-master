...
//  Arduino-TVOS-BLE
//
//  Created by Dr. Atta on 01/05/2020.
...
class ViewController: UIViewController {
	...
	var detailViewController : ControlsTableViewController!
	...
	override func viewDidLoad() {
		...
		detailViewController = self.children[0] as? ControlsTableViewController
	...
	fileprivate func setDetailVisibility(_ isVisible: Bool){
		DispatchQueue.main.async {
			self.detailContainerView.isHidden = !isVisible
			self.detailContainerView.isUserInteractionEnabled = isVisible
		}
	}
...
extension ViewController: UITableViewDelegate, UITableViewDataSource{
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		cell.textLabel?.text = self.nearbyPeripheralInfos[indexPath.row].peripheral.name ?? "Unknown Device"
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		let peripheralToConnect = nearbyPeripheralInfos[indexPath.row].peripheral
		
		if peripheralToConnect == discoveredPeripheral {return}
